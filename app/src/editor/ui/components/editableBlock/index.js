...
const isControlOrCommandKey = (key) => key === 'Control' || key === 'Meta';
...
class EditableBlock extends React.Component {
  ...
  takeScreenshot() {
    const videoElem = document.querySelector('iframe').contentWindow.document;
    console.log(videoElem)
    // html2canvas(videoElem).then((canvas) => {
  ...
  handleKeyDown(e) {
    if ((e.metaKey || e.ctrlKey) && e.key === SLASH_KEY) {
      this.takeScreenshot();
    } else if (e.key === SLASH_KEY) {
